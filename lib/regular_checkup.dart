...
class _RegularCheckupScreenState extends State<RegularCheckupScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Regular Checkup"), backgroundColor: Colors.teal.shade900),
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 10),
            CheckboxListTile(title: const Text("Fever?"), value: hasFever, onChanged: (v) => setState(() => hasFever = v ?? false)),
            CheckboxListTile(title: const Text("Cough?"), value: hasCough, onChanged: (v) => setState(() => hasCough = v ?? false)),
            CheckboxListTile(title: const Text("Headache?"), value: hasHeadache, onChanged: (v) => setState(() => hasHeadache = v ?? false)),
  ...
  Future<void> _submitCheckup() async {
    ...
    if (unsafe) {
      ...
      final doctorEmail = "abbasameesha@gmail.com"; // replace with real doctor email
      await sendMishapEmail(userEmail, doctorEmail, feedback);
    }
  ...
  Future<void> sendMishapEmail(String fromEmail, String toEmail, String report) async {
    final smtpServer = gmail('abbasameesha@gmail.com', '/Ameesha1*Abbas1/'); // Use App Password
    final message = Message()
      ..from = Address(fromEmail, 'Health App')
    ...
    try {
      ...
      print('Mishap email sent: ' + sendReport.toString());
